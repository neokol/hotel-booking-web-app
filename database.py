...
def login_user(name, pswd):
    with engine.connect() as conn:
        query = text("SELECT * FROM user WHERE email = :email AND password = :pswd")
...
def register_user(name, email, password):
    with engine.connect() as conn:
        ...
        if account:
            ...
            conn.execute(
                insert_query, {"name": name, "email": email, "password": password}
            )
            # commit changes
            conn.commit()
...
def add_user_review(user_id, room_id, rate, comment):
    with engine.connect() as conn:
        ...
        res = conn.execute(
            query,
            {"user_id": user_id, "room_id": room_id, "rate": rate, "comment": comment},
        )
        conn.commit()
        return res
